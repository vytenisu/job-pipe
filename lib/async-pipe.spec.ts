import { createAsyncPipe, AsyncPipeQueueExceeded, AsyncPipeAborted } from './async-pipe';

describe('Async pipe', () => {
  it('executes a job', async () => {
    const pipe = createAsyncPipe();
  ...
  it('executes multiple jobs at the same time if pipe is wide enough', () => {
    const pipe = createAsyncPipe({ throughput: 2 });
  ...
  it('cancels job if there is no waitlist', async () => {
    const pipe = createAsyncPipe({ throughput: 2, maxQueueSize: 0 });
    ...
    expect(error).toBeInstanceOf(AsyncPipeQueueExceeded);
  ...
  it('cancels older jobs if waitlist is not sufficient', async () => {
    const pipe = createAsyncPipe({ throughput: 1, maxQueueSize: 1 });
  ...
  it('does not interfere with multiple job execution if pipe remains not filled', () => {
    const pipe = createAsyncPipe({ throughput: 4 });
  ...
  it('provides current queue length', () => {
    const pipe = createAsyncPipe({ throughput: 1, maxQueueSize: 4 });
  ...
  it('allows rejecting all jobs', async () => {
    ...
    expect(error1).toBeInstanceOf(AsyncPipeAborted);
    expect(error2).toBeInstanceOf(AsyncPipeAborted);
    expect(error3).toBeInstanceOf(AsyncPipeAborted);
    expect(error4).toBeInstanceOf(AsyncPipeAborted);
